"""
MM256 V2 workflow orchestrator — with feature engineering.

Same 4-step pipeline as workflow_mm256.py but uses the V2 preprocessor
which applies competition-inspired feature engineering (lags, deltas,
rolling stats, slopes, interaction features) before windowing.

Default flow:
  1. Load and preprocess with V2 feature engineering (unscaled).
...
Key difference from V1: catch22 is disabled by default since hand-engineered
features replace its role.
"""
...
from interface.workflow_mm256 import split_temporal_holdout
...
def run_pipeline_mm256_v2(
    ...
    """Run the full MM256 V2 benchmark workflow with feature engineering."""
    started = perf_counter()

    print(f"\n{'='*60}")
    print(f"  MM256 V2 Workflow — iteration {iteration}")
    print("  holdout split -> CV on train only -> final train -> holdout test")
    print(f"{'='*60}\n")

    # Step 1: Load & preprocess with V2 feature engineering
    step_t = perf_counter()
    print("Step 1/4 — Loading & V2 preprocessing (unscaled)...")
    data, _, preprocessing_meta = preprocess_mm256_v2(
        source=source,
        cache_raw=cache_raw,
    ...
    # Step 4: Final training on full train split + holdout test
    step_t = perf_counter()
    ...
    print(f"\n{'='*60}")
    print(f"  MM256 V2 workflow complete in {_fmt(total)}")
    print(f"  Iteration: {iteration}  |  Features: {preprocessing_meta['n_features']}")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="MM256 V2 workflow with feature engineering")
    ...
    parser.set_defaults(use_catch22=False)  # V2 default: catch22 disabled
    parser.add_argument("--iteration", type=int, choices=[1, 2], default=1,
                        help="Feature iteration: 1=core, 2=extended with pressure+directions")
    args = parser.parse_args()

    if args.mode == "cv":
        run_cv_pipeline_mm256_v2(
            source=args.source,
